      ...
      const htmlContent = `
        ...
        <head>
          <style>
            ...
            .logo { font-size: 24px; font-weight: bold; color: #16A085; }
            .section { margin-bottom: 30px; }
            h2 { color: #16A085; font-size: 18px; margin-bottom: 15px; }
            .score-box { background-color: #F8F9FA; padding: 20px; border-radius: 8px; text-align: center; border: 1px solid #E9ECEF; }
            .score-value { font-size: 48px; font-weight: bold; color: #2C3E50; }
            table { width: 100%; border-collapse: collapse; margin-top: 10px; }
            th, td { padding: 12px; text-align: left; border-bottom: 1px solid #E9ECEF; }
            th { background-color: #F8F9FA; color: #495057; font-weight: 600; }
            .footer { text-align: center; margin-top: 50px; font-size: 12px; color: #6C757D; border-top: 1px solid #E9ECEF; padding-top: 20px; }
          </style>
        </head>
        <body>
          <div class="header">
            <div class="logo">Halo Health</div>
          ...
          <div class="section score-box">
            ...
            <div>Based on ${totalScans} product scans</div>
          </div>
          ...
          <div class="section">
            ...
            <table>
              ...
              ${CATEGORIES.map(cat => `<tr><td>${cat.label}</td><td>${categoryScores[cat.key] || categoryScores[cat.label] || 0}</td></tr>`).join('')}
            </table>
          </div>

          <div class="section">
            <h2>Top Flagged Ingredients</h2>
            <table>
              <tr><th>Ingredient</th><th>Flag Count</th></tr>
              ${topIngredients.slice(0, 10).map(item => `<tr><td>${item.name || item}</td><td>${item.count || 1}</td></tr>`).join('')}
            </table>
          </div>

          <div class="footer">
            This report is generated by Halo Health for personal informational purposes only and does not constitute medical advice.
          </div>
        </body>
        </html>
      `;
  ...
  return (
    <SafeAreaView style={styles.safe} edges={['top']}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Health Reports</Text>
        <TouchableOpacity style={styles.backBtn} onPress={generatePDF}>
